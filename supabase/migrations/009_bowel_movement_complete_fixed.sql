-- Combined Migration: Complete Bowel Movement Tracking Setup (with duplicate cleanup)
...
-- ============================================================
-- Part 2.5: Clean up duplicates and add unique constraint
-- ============================================================
...
BEGIN
    ...
    FROM (
        SELECT user_id, recorded_date, COUNT(*) as cnt
        FROM daily_symptom_entries
        GROUP BY user_id, recorded_date
        HAVING COUNT(*) > 1
    ) duplicates;

    RAISE NOTICE 'Found % duplicate (user_id, recorded_date) combinations', duplicate_count;
END $$;

-- Step 2: Keep only the most recent entry for each (user_id, recorded_date) combination
-- Delete older duplicates based on created_at timestamp
DELETE FROM daily_symptom_entries
WHERE id IN (
    SELECT id
    FROM (
        SELECT
            id,
            ROW_NUMBER() OVER (
                PARTITION BY user_id, recorded_date
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'Created unique constraint on (user_id, recorded_date)';
    ELSE
